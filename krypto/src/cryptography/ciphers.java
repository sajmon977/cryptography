...
public class ciphers {
    ...
    public static String text(String text) {
        ...
        for (int i = 0; i < text.length(); i++) {
            if (('a' <= text.charAt(i) && text.charAt(i) <= 'z') || ('A' <= text.charAt(i) && text.charAt(i) <= 'Z')) {
                ans += (text.charAt(i) <= 'Z') ? text.charAt(i) : (char) (text.charAt(i) + 'A' - 'a');
            } else if (text.charAt(i) != ' ') {
                throw new java.lang.Error("wrog character: " + text.charAt(i));
            }
        }
        return ans;
    }

    public static String cezar(String text_, int shift, boolean encode) {
        String ans = "", text = text(text_);

        for (int i = 0; i < text.length(); i++) {
            ans += encode ? mod(text.charAt(i) + shift) : mod(text.charAt(i) - shift);
        }
        return ans;
    }

    public static String vigener(String text_, String key, boolean encode) {
        String ans = "", text = text(text_);

        for (int i = 0; i < text.length(); i++) {
            ans += encode ? mod(text.charAt(i) + (key.charAt(i % key.length()) - 'A')) : mod(text.charAt(i) - (key.charAt(i % key.length()) - 'A'));
        }
        return ans;
    }

    public static String substitution(String text_, String key, boolean encode) {
        String ans = "", text = text(text_);
    ...
    public static String permutation(String text_, String key, boolean encode) {
        String ans = "", text = text(text_);
        int m = key.length();
